...
bool check(char* word)
{
    //creare cursor e index come sopra. cursor sarà il pointer che scorre tra la linked list; se non è null vuol dire che quella lettera c'è
    // tra le parole; quando finisco le lettere di word controllo che l'ultimo cursor abbia come bool true, che significa che la parola è finita;
    // altrimenti è come se cercassi "sotto" in "sottopanca";

    //così però il while le scorre tutte mentre il for rimane alla prima lettera. dovrei mettere un i++ alla fine del while loop e usare
    // i all'interno partendo da 0, così incrementano insieme;
    node *cursor = root;
    for (int i = 0; i < strlen(word); i++)
    {
        ...
        {cursor = cursor->children[index];} else{return false;}
    }
    if (cursor->is_word == true) {return true;} else{ return false;}
}
